...
| 欄位 | 型別 | 必填 | 說明 | 備註 |
|---|---|---|---|---|
| `prober` | string | 是 | 指定探測器類型 | 只允許 `http` `tcp` `grpc` `icmp` `dns` `ntp` |
...
| `ntp` | object | `prober: ntp` 時使用 | NTP probe 設定 | 自家延伸 schema，不走 upstream config parser |
...
| `enable_http3` | bool | 否 | 是否啟用 HTTP/3 | `v0.28.0` 新增欄位，需為 bool |
...
| `compression` | string | 否 | Accept-Encoding / 解壓設定 | validator 接受字串 |
| `body_size_limit` | string 或 number | 否 | 回應 body 限制 | validator 接受字串或數字 |
...
| `ip_protocol_fallback` | bool | 否 | 是否允許 fallback |
| `tls` | bool | 否 | 是否使用 TLS |
| `metadata` | map[string][]string | 否 | gRPC metadata / header |
| `tls_config` | object | 否 | TLS 設定，結構同 HTTP TLS |
...
| `ip_protocol_fallback` | bool | 否 | 是否允許 fallback |  |
| `source_ip_address` | string | 否 | 指定來源 IP |  |
| `protocol_version` | int | 否 | NTP protocol version | 允許 `2` `3` `4` |
| `measurement_duration` | string | 否 | 高 drift 追加量測時間 | 例如 `30s` |
| `high_drift_threshold` | string | 否 | 高 drift 門檻 | 例如 `10ms` |
